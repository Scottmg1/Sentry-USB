...
# Shared with the Go server's SyncToArchive(): both update the same cache at
# DRIVE_DATA_SYNC_CACHE so the guard stays consistent across CIFS/NFS (Go)
...
function drive_data_size_guard_ok() {
  ...
  # new >= last/2 → allow (integer math; boundary matches Go-side).
...
# Called after every successful sync (Go-side writes the same file).
