...
then
  ...
  then
    ...
    cat <<- EOF > /etc/rc.local
		#!/bin/bash
		{
		  while ! curl -s https://raw.githubusercontent.com/${REPO}/${REPO_NAME}/${BRANCH}/setup/generic/install.sh
		  do
		    sleep 1
		  done
		} | bash
    ...
    {
      while ! curl -s https://raw.githubusercontent.com/${REPO}/${REPO_NAME}/${BRANCH}/tools/debian-resizefs.sh
      do
        sleep 1
      done
    } | bash -s "${target_gb}G"
...
then
  while ! curl -o /sentryusb/sentryusb.conf https://raw.githubusercontent.com/${REPO}/${REPO_NAME}/${BRANCH}/pi-gen-sources/00-sentryusb-tweaks/files/sentryusb.conf.sample
  do
    sleep 1
  done
fi
...
then
  ...
  then
    while ! curl -o /sentryusb/wpa_supplicant.conf.sample https://raw.githubusercontent.com/${REPO}/${REPO_NAME}/${BRANCH}/pi-gen-sources/00-sentryusb-tweaks/files/wpa_supplicant.conf.sample
    do
      sleep 1
    done
  fi
...
rm -f /etc/rc.local
while ! curl -o /etc/rc.local https://raw.githubusercontent.com/${REPO}/${REPO_NAME}/${BRANCH}/pi-gen-sources/00-sentryusb-tweaks/files/rc.local
do
  sleep 1
done
chmod a+x /etc/rc.local
...
then
  cat <<- EOF >> /root/.bashrc
	...
	then
	  ...
	  echo "| https://github.com/${REPO}/${REPO_NAME}/blob/${BRANCH}/doc/OneStepSetup.md  |"
